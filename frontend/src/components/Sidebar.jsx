...
// permission → nav item id eşleşmesi
const PERM_MAP = {
  dashboard:          'dashboard',
  kanal_analizi:      'channels',
  ai_raporlar:        'report',
  butce_planlama:     'budget',
  tv_ad_verification: 'tv',
  tv_medya_plani:     'tvplan',
...
export default function Sidebar({ active, onNav, onLogout }) {
  ...
  const isVisible = (item) => {
    if (!item.perm) return true; // perm=null → her zaman görünür
    ...
    <aside className="sidebar">
      ...
      {isAgency && selectedBrand && (
        <div style={{ margin: '0 12px 8px', padding: '10px 12px', background: 'rgba(0,191,166,0.07)', border: '1px solid rgba(0,191,166,0.2)', borderRadius: 10 }}>
          ...
          <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text1)', lineHeight: 1.3 }}>{selectedBrand.name || selectedBrand.company_name}</div>
          <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>Marka</div>
        </div>
      )}
                ...
                >
                  ...
                  {item.badge && (
                    <span style={{
                      ...
                      padding: '2px 6px',
                      borderRadius: 4,
                      background: item.badgeLive ? 'rgba(255,107,90,0.2)' : 'rgba(0,191,166,0.15)',
      ...
      <div className="sidebar-footer">
        <div style={{ fontSize: 12, color: 'var(--text2)', fontWeight: 600, marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {user?.company_name}
        </div>
        <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 10 }}>
          {user?.is_company_admin ? 'Şirket Yöneticisi' : user?.company_type === 'agency' ? 'Ajans' : 'Marka'}
        </div>
        {onLogout && (
          <button onClick={onLogout}
            style={{ width: '100%', padding: '7px 0', background: 'transparent', border: '1px solid var(--border2)', borderRadius: 8, color: 'var(--text3)', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
            Çıkış Yap
          </button>
        )}
      </div>
