...
const ICONS = {
  ...
  logs:         <><rect x="5" y="3" width="14" height="18" rx="2"/><line x1="9" y1="8" x2="15" y2="8"/><line x1="9" y1="12" x2="15" y2="12"/><line x1="9" y1="16" x2="13" y2="16"/></>,
  pricing:      <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-4H9l3-3 3 3h-2v4zm0-8h-2V7h2v2z"/>,
...
const AGENCY_BASE_NAV = [
  ...
  { label: 'Yönetim', items: [
    ...
    { id: 'logs',          label: 'İşlem Kayıtları', perm: null, adminOnly: true },
...
export default function Sidebar({ active, onNav, onLogout, open, onClose }) {
  ...
  const isVisible = (item) => {
    if (item.adminOnly && !user?.is_company_admin && !user?.is_platform_admin) return false;
    if (item.id === 'subscription') {
      if (user?.company_type === 'agency') return !selectedBrand;
      if (user?.company_type === 'brand') return !user?.is_managed_by_agency;
      return false;
    }
